...
def detect_unsafe_zone(video_path, zone_vertices=None):
    ...
    if zone_vertices is None:
        # Adjusted polygon to actually fit within the 960x540 resized frame (previous Y coords of 650 were out of bounds)
    ...
    try:
        while True:
            ...
            # Require at least 45% confidence for a person to avoid ghost detections triggering the zone
            results = _model(frame, conf=0.45, verbose=False)
