    ...
    <aside className="w-64 bg-white/60 dark:bg-[#18181b]/60 backdrop-blur-2xl border-r border-gray-200/50 dark:border-white/5 transition-colors duration-300 flex flex-col hidden md:flex sticky top-0 h-screen shadow-[4px_0_24px_rgb(0,0,0,0.02)]">
      ...
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
        ...
        {navItems.map((item) => (
          <Link key={item.name}
            to={item.path}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${activeTab === item.name ? "bg-[#0066CC] dark:bg-[#E8A33D] text-white dark:text-black shadow-md shadow-[#0066CC]/20 dark:shadow-[#E8A33D]/20" : "text-gray-600 dark:text-gray-400 dark:text-gray-400 hover:bg-black/5 dark:hover:bg-white/5 hover:text-gray-900 dark:text-gray-200 dark:hover:text-white"}`}>
    ...
    <main className="flex-1 flex flex-col min-h-screen overflow-hidden relative">
      ...
      <header className="h-20 bg-white/40 dark:bg-[#18181b]/40 backdrop-blur-xl border-b border-gray-200/50 dark:border-white/5 transition-colors duration-300 dark:border-white/5 flex items-center justify-between px-8 sticky top-0 z-10">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Real-time metrics from video detections</p>
        </div>
      </header>
      {/* Dashboard Content */}
      <div className="p-8 overflow-y-auto flex-1 z-0 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white/60 dark:bg-[#18181b]/60 backdrop-blur-xl rounded-[1.5rem] transition-colors duration-300 p-6 border border-white dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.03)] flex items-center justify-between hover:scale-[1.02] transition-transform">
            <div>
              <p className="text-sm font-semibold text-gray-500 tracking-tight">Active Workers</p>
              <h3 className="text-3xl font-bold text-[#1d1d1f] dark:text-white mt-2 tracking-tight">{statsData.workers}</h3>
            </div>
          ...
          <div className="bg-white/60 dark:bg-[#18181b]/60 backdrop-blur-xl rounded-[1.5rem] transition-colors duration-300 p-6 border border-white dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.03)] flex items-center justify-between hover:scale-[1.02] transition-transform">
            <div>
              ...
              <h3 className="text-3xl font-bold text-[#1d1d1f] dark:text-white mt-2 tracking-tight">{statsData.compliance_score}%</h3>
            </div>
          ...
          <div className="bg-white/60 dark:bg-[#18181b]/60 backdrop-blur-xl rounded-[1.5rem] transition-colors duration-300 p-6 border border-white dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.03)] flex items-center justify-between hover:scale-[1.02] transition-transform">
            <div>
              ...
              <h3 className="text-3xl font-bold text-[#1d1d1f] dark:text-white mt-2 tracking-tight">{statsData.total_incidents}</h3>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white/60 dark:bg-[#18181b]/60 backdrop-blur-xl rounded-[1.5rem] transition-colors duration-300 border border-white dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.03)] p-8">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-lg font-semibold text-[#1d1d1f] dark:text-white tracking-tight flex items-center gap-2">
          ...
          <div className="bg-white/60 dark:bg-[#18181b]/60 backdrop-blur-xl rounded-[1.5rem] transition-colors duration-300 border border-white dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.03)] p-8">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-lg font-semibold text-[#1d1d1f] dark:text-white tracking-tight flex items-center gap-2">
                <FaChartBar className="text-[#E8A33D] dark:text-[#E8A33D]" /> Incident Breakdown
              </h2>
            </div>
