...
def _init_model():
    ...
    model_path = os.path.normpath(os.path.join(BASE_DIR, "..", "models", "ppe_model.pt"))
...
def detect_vests(video_path):
    ...
    try:
        while True:
            ...
            # Set conf=0.45 and iou=0.5 to significantly reduce false positives and increase overall accuracy
            results = _model(frame, conf=0.45, iou=0.5, verbose=False)
            ...
            for result in results:
                ...
                for box in boxes:
                    ...
                    # PERSON (class 5), SAFETY VEST (class 7)
                    if cls_id == 5:
                        worker_count += 1
                    elif cls_id == 7:
                        vest_count += 1
            
            no_vest_count = max(0, worker_count - vest_count)
