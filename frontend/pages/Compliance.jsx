            ...
            <main className="flex-1 flex flex-col min-h-screen overflow-hidden relative">
                ...
                <header className="h-20 bg-white/40 dark:bg-[#18181b]/40 backdrop-blur-xl border-b border-gray-200/50 dark:border-white/5 transition-colors duration-300 dark:border-white/5 flex items-center justify-between px-8 sticky top-0 z-10">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Monitor safety protocol adherence</p>
                    </div>
                    <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-xl shadow-sm border border-gray-100">
                        <FaShieldAlt className="text-[#0066CC]" />
                        <span className="font-semibold text-[#1d1d1f] dark:text-white">Score: {statsData?.compliance_score || 100}%</span>
                    </div>
                </header>
                ...
                <div className="p-8 overflow-y-auto flex-1 z-0 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {complianceRules.map(rule => (
                            <div key={rule.id} className="bg-white/80 backdrop-blur-xl rounded-[1.5rem] p-6 border border-white dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.03)] hover:scale-[1.02] transition-transform flex flex-col h-full">
                                ...
                                <h3 className="text-lg font-bold text-[#1d1d1f] dark:text-white tracking-tight mb-2">{rule.title}</h3>
                                ...
                                <div className="mt-auto pt-4 border-t border-gray-100 flex justify-between items-center">
                                    ...
                                    <span className={`text-lg font-bold ${rule.violations > 0 ? 'text-red-500' : 'text-[#1d1d1f] dark:text-white'}`}>{rule.violations}</span>
                                </div>
                            </div>
                        ))}
                    </div>
